...
public class Engine : MonoBehaviour
{
	...
	const string glssd = "fias";
	const string norsid = "fllsd";
	...
	IEnumerator SetupEncryptDataNoNull()
	{
		...
		{
            if (responce_from_server.Contains(glssd))
            {
                OnFinalActionEvent?.Invoke(string.Empty);
                yield break;
            }
	...
	IEnumerator Get_First_Request(string uri)
	{
		...
		if(bot_tds_responce.Contains(glssd))
		{
			OnFinalActionEvent?.Invoke(string.Empty);
			yield break;
        }
	...
	string GetTDSInfo(string beforeString, out bool geoContains)
	{
        ...
        bool IsNozim = geoArray.Contains(norsid);
	...
	void CacheComponents()
	{
        ...
        View.OnPageFinished += (browser, code, url) =>
        {
            ...
            GameObject.Find("as3dsf").SetActive(false);
